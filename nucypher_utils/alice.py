...
def run():
    ...
    # If we had an existing alice in disk, let's get it from there

    passphrase = "TEST_alice_INSECURE_DEVELOPMENT_PASSWORD"
    # If anything fails, let's create alice from scratch
    ...
    # At this point, alice is fully operational and can create policies.
    # The Policy Label is a bytestring that categorizes the data that alice wants to share.
    ...
    # alice can create the public key associated to the policy label,
    ...
    # alice now wants to share data associated with this label.
    ...
    # - m-out-of-n: This means alice splits the re-encryption key in 5 pieces and
    #               she requires Bob to seek collaboration of at least 3 Ursulas
    m, n = 2, 3


    # With this information, alice creates a policy granting access to Bob.
    # The policy is sent to the NuCypher network.
    print("Creating access policy for the Doctor...")
    policy = alice.grant(bob=doctor_strange,
                        label=label,
                        m=m,
                        n=n,
